...
namespace YiSha.CodeGenerator.Template
{
    public class SingleTableTemplate
    {
        ...
        public BaseConfigModel GetBaseConfig(string path, string userName, string tableName, string tableDescription, List<string> tableFieldList)
        {
            ...
            baseConfigModel.PageIndex.ColumnList = tableFieldUpperList.Take(defaultField).ToList();
            ...
            baseConfigModel.PageForm.FieldList = tableFieldUpperList.Take(defaultField).ToList();  
            #endregion
        ...
        public string BuildIndex(BaseConfigModel baseConfigModel, DataTable dt)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                column = TableMappingHelper.ConvertToUppercase(column);
                Tuple<string, string> tuple = new Tuple<string, string>(column, remark);
                tuples.Add(tuple);
            }
            #region 初始化集合
            ...
            if (baseConfigModel.PageIndex.IsSearch == 1)
            {
                ...
                string fieldTitle = tuples.FirstOrDefault(p => p.Item1 == fieldName).Item2;
                if (fieldTitle.IsEmpty()) { fieldTitle = fieldName; }
                ...
                sb.AppendLine("                <ul>");
                sb.AppendLine("                    <li>");
                sb.AppendLine("                        " + fieldTitle + "：<input id=\"" + fieldNameLower + "\" col=\"" + fieldName + "\" type=\"text\" />");
                sb.AppendLine("                    </li>");
                sb.AppendLine("                    <li>");
            ...
            {
                string title = tuples.FirstOrDefault(p => p.Item1 == column).Item2;
                if (title.IsEmpty()) { title = column; }
                sb.AppendLine("                { field: '" + column + "', title: '" + title + "' },");
            }
                ...
                switch (baseConfigModel.PageForm.ShowMode)
                {
                    case 1:
                        for (int i = 0; i < baseConfigModel.PageForm.FieldList.Count; i++)
                        {
                            ...
                            fieldTitle = Convert.ToString(tuples.FirstOrDefault(p => p.Item1 == field).Item2);
                            if (fieldTitle.IsEmpty()) { fieldTitle = field; }
                            fieldLower = TableMappingHelper.FirstLetterLowercase(field);

                            sb.AppendLine("        <div class=\"form-group\">");
                            sb.AppendLine("            <label class=\"col-sm-3 control-label \">" + fieldTitle + (i == 0 ? "<font class=\"red\"> *</font>" : string.Empty) + "</label>");
                    ...
                    case 2:
                        for (int i = 0; i < baseConfigModel.PageForm.FieldList.Count; i++)
                        {
                            ...
                            sb.AppendLine("            <label class=\"col-sm-2 control-label \">" + fieldTitle + "<font class=\"red\"> *</font></label>");
